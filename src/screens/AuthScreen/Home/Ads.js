			...
			<ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
				<Image
					style={{
						...
						borderRadius: 5,
						height: 170,
						marginLeft: SizeList.bodyPadding,
					}}
				...
				<Image
					...
					source={require("src/assets/images/Banner2.jpg")}
				/>
			</ScrollView>
			<Text
				style={{ margin: SizeList.bodyPadding, marginTop: 0 }}
				font="SemiBold">
				Tahukah Kamu?
			</Text>
			{loading ? (
					...
					renderItem={({ item, index }) => (
						<CardTextImage
							style={{
								marginLeft: SizeList.bodyPadding,
								marginRight:
									index == newsData.length - 1 ? SizeList.bodyPadding : 0,
							}}
							onPressCard={() =>
								navigation.navigate("/news-screen", {
									title: item.title.rendered,
									data: item.content.rendered,
									newsImage: item.jetpack_featured_media_url,
									link: item.link,
								})
							}
