...
FormData.prototype.appendObject = function (obj, except) {
	...
	for (var key in obj) {
		if (!except.Contains(key)) this.append(key, obj[key])
	}
}
Array.generateEmpty = function (length, empty) {
	return Array.from(new Array(length), function (a, i) {
		return empty ? "" : i
	})
}
...
Array.prototype.loopCallback = function (callback, reverse, index) {
	...
	function retCallbackReverse(timeout) {
		timeout = timeout || 0
		setTimeout(function () {
			if (index > 0) arr.loopCallback(callback, reverse, index)
		}, timeout)
	}
	if (reverse) {
...
String.prototype.extractNumber = Number.prototype.extractNumber = function () {
	...
	try {
		...
		if (matches.length > 0) return matches.join("").replace(/\s/g, "").toInt()
	} catch (e) {
		return 0
	}
	return 0
