...
export const convertPhoneNumber = num => {
	...
	const hasil = temp_hasil.join("-").split("").reverse().join("")
	let final = hasil
	return final
}
...
export const getFavorites = async page => {
	...
	const { data } = await axios.get(
		`${HOST_URL}/favorites/${userId}?page=${page}`,
		{
...
export const editCategory = async (data, id_category) => {
	...
	const res = await axios.post(
		`${HOST_URL}/category_update/${id_category}`,
		data,
		{
			headers: { authorization: userToken },
		},
...
export const editCustomer = async (data, id_cust) => {
	...
	try {
		const res = await axios.post(
			`${HOST_URL}/customer_update/${id_cust}`,
			data,
			{
				headers: { authorization: userToken },
			},
...
export const payCredit = async (data, transactionId) => {
	...
	try {
		const res = await axios.post(
			`${HOST_URL}/pay_debt/${transactionId}`,
			data,
			{
				headers: { authorization: userToken },
			},
