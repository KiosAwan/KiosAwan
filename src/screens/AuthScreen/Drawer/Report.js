...
const ViewShadow = props => (
	<View
		style={{
			...
			padding: props.noPadding ? 0 : SizeList.padding,
		}}>
		{!props.noTitle && (
			<Text style={{ marginBottom: props.noMargin ? 10 : 0 }}>
				{props.title}
			</Text>
		)}
		<View
			{...props}
			style={[
				{ marginTop: props.noMargin ? 0 : SizeList.base },
				shadowStyle,
				props.style,
			]}
		/>
...
const Report = ({ navigation }) => {
	...
	const GetData = async param => {
		const { data: dataReportCategory } = await getReportCategory(
			User.store.id_store,
			param,
		)
		const { data: dataTransaction } = await getTransactionData(
			User.store.id_store,
			param,
		)
		const { data: dataReportNonTunai } = await getReportNonTunai(
			User.store.id_store,
			param,
		)
	...
	return (
		<Container>
						...
						content={close => (
							<DateRangePicker
								initialRange={initRange(
									dateRange.length > 0 ? dateRange : undefined,
								)}
								onSuccess={(from, to) => {
			...
			<Body
				refreshControl={
					<RefreshControl refreshing={false} onRefresh={_handleRefresh} />
				}>
				{!User.store ? (
					...
					<View>
						<View style={{ marginBottom: SizeList.base }}>
							<SelectBoxModal
								...
								renderItem={(title, i) => (
									<Text
										color={i == SelectBox.index ? "primary" : "greyFont"}
										font="SemiBold">
										{title.toUpperCase()}
									</Text>
								)}
						...
						<ViewShadow
							noPadding
							title={initRange(dateRange.length > 0 ? dateRange : undefined)
								.map(date => {
							...
							<Wrapper style={{ marginVertical: SizeList.base }}>
								<View>
									...
									<Wrapper>
										<Text color={_getFlagColor(dataTransaction.penjualan_flag)}>
											{_convertRupiah(dataTransaction, "total_penjualan")}
										</Text>
										{dataTransaction.penjualan_flag != 0 && (
											<Icon
												size={15}
												name={
													dataTransaction.penjualan_flag == 2
														? "sort-up"
														: "sort-down"
												}
								...
								<View>
									...
									<Wrapper>
										<Text color={_getFlagColor(dataTransaction.profit_flag)}>
											{_convertRupiah(dataTransaction, "total_profit")}
										</Text>
										{dataTransaction.profit_flag != 0 && (
											<Icon
												size={15}
												name={
													dataTransaction.profit_flag == 2
														? "sort-up"
														: "sort-down"
												}
							...
							<Wrapper style={{ marginVertical: SizeList.base }}>
								<View>
									...
									<Text align="center" color="primary">
										{dataTransaction && dataTransaction.jumlah_transaksi}
									</Text>
								</View>
								<View>
									<Text align="center">Produk Terjual</Text>
									<Text align="center" color="primary">
										{dataTransaction && dataTransaction.produk_terjual}
									</Text>
								</View>
							</Wrapper>
						</ViewShadow>
							...
							{dataTransaction &&
								[
									...
									<Wrapper
										...
										<Text color={i == 0 && "primary"}>
											{_convertRupiah(dataTransaction, key)}
										</Text>
									</Wrapper>
								))}
						</ViewShadow>
						...
						<SelectBoxModal
							...
							renderItem={item => (
								<Text
									font="SemiBold"
									color={
										NT.selected && NT.selected.method == item.method
											? "primary"
											: "greyFontHard"
									}>
									...
									<Wrapper
										...
										<Text color={i == 0 && "primary"}>
											{key == "jumlah_transaksi"
												? NT.selected[key]
												: _convertRupiah(NT.selected, key)}
										</Text>
									</Wrapper>
								))}
						</ViewShadow>
						{dataReportCategory &&
							dataReportCategory.rMap(
								({ harga, data, nama_category }, index) => (
									<ViewShadow
										noMargin
										noPadding
										noTitle={index != 0}
										...
										<Text
											...
											{!nama_category
												? "PESANAN MANUAL"
												: nama_category.toUpperCase()}
										</Text>
										<Divider style={{ marginVertical: SizeList.secondary }} />
										{data && data.length > 0 ? (
											data.rMap(({ Product, harga_jual, total, jumlah }, i) => (
												<Wrapper
													style={{
														marginBottom: SizeList.secondary,
														paddingVertical: SizeList.secondary,
													}}
													spaceBetween>
													<View _width="68%">
														<Text>{Product}</Text>
														<Text>
															{harga_jual.convertRupiah()} x {jumlah}
														</Text>
													</View>
													<Text _width="30%" _flexStart align="right">
														{total.convertRupiah()}
													</Text>
												</Wrapper>
											))
