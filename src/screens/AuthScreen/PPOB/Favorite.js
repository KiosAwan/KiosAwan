...
const Favorite = ({ navigation }) => {
			...
			<View>
				<View
					...
					<View style={{ marginVertical: SizeList.base }}>
						<Button
							onPress={() => navigation.navigate(`/ppob/${type}`, data)}
							color="link"
							...
							<View _flex>
								...
								<Text>
									{customer_name
										? customer_name.ucwords()
										: type.split("_").join(" ").ucwords()}
								</Text>
							</View>
							<Button
								padding={0}
								color="link"
								onPress={() => removeFavorite(id_favorite)}>
								<Image
									size={25}
	...
	const renderGroupValue = v => {
		if (v == "pdam") return "PDAM"
		return v == "~" ? "Semua Produk" : v.split("_").join(" ").ucwords()
	}
	const dataFiltered = () =>
		favorites.filter(
			({ type }) => state.selected == "~" || state.selected == type,
		)
	return (
		<Container>
			<GlobalHeader
			...
			<BottomSheetSelect
				...
				renderItem={(item, i) => (
					<Text font="SemiBold" style={i == 0 && { marginTop: SizeList.base }}>
						{renderGroupValue(item).toUpperCase()}
					</Text>
				)}
			/>
			{isLoading ? (
