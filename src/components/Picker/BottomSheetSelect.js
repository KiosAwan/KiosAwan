	...
	const render = () => {
		return isSelect ? (
			<View style={{ flex: 1 }}>
				...
				<View style={{ padding: SizeList.base }}>{footer}</View>
			</View>
		) : (
			<View style={{ flex: 1, padding: SizeList.base }}>
				{typeof sheetContent == "function"
					? sheetContent(() => rb.close())
					: sheetContent}
			</View>
		)
	}
...
const BottomSheet = _props => {
	const { content, renderButton, btnStyle: btnStyleOverride, ...props } = _props
	return (
		<BottomSheetSelect
			btnProps={{
				flexStart: true,
...
BottomSheetSelect.propTypes = {
	...
	sheetContent: PropTypes.oneOfType([PropTypes.element, PropTypes.func]),
}
