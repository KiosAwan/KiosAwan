...
const RiwayatTransaksi = ({ navigation }) => {
	...
	const _addMoreData = async () => {
		if (
			parseInt(RiwayatTransaksi.nextPage) < parseInt(RiwayatTransaksi.total)
		) {
			...
			{RiwayatTransaksi.isLoading ? (
				...
				<View
					...
					<Image
						...
						source={require("src/assets/images/riwayat.png")}
					/>
					<Text font="Bold" size={16}>
						Anda belum memiliki riwayat
					</Text>
					<Text style={{ marginTop: 20 }} align="center">
						Silahkan lakukan topup atau pembelian produk pulsa dan tagihan
					</Text>
				</View>
			) : (
							...
							renderItem={({ item, index }) => (
								<Fragment>
									<Wrapper justify="space-between">
										<View _width="100%">
											<Wrapper justify="space-between">
												<Text _width="65%" font="SemiBold" color="primary">
												...
												<Text
													_width="35%"
													align="right"
													color={item.type == 0 ? "success" : "danger"}>{`${
													item.type == 0 ? "+" : "-"
												} ${convertRupiah(item.amount)}`}</Text>
											</Wrapper>
									...
									{index != RiwayatTransaksi.data.length - 1 && (
