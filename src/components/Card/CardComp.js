	...
	return (
		<TouchableOpacity onPress={props.onPressCard}>
			<View style={[styles.CardTextImage, props.style]}>
				<Image
					style={styles.imageCardTextImage}
					source={{ uri: props.image }}
				/>
				<View style={styles.textImageWrapper}>
					<Text
						style={{
							fontSize: FontList.titleSize,
...
export const ImageText = props => {
	return (
		<View
			style={[
				styles.viewNoImageProduct,
				{
					width: props.size || width * 0.17,
					height: props.size || width * 0.17,
				},
				props.style,
			]}>
			<Text font="SemiBold" color="greyFont" size={20}>
				{props.name
					? props.notGenerated
						? props.name
						: props.name.generateInitial()
					: null}
			</Text>
		</View>
	)
...
export const ProductCard = props => {
	...
	return (
		<TouchableOpacity
			onPress={onPressPlus ? (plusDisabled ? null : onPressPlus) : null}
			activeOpacity={onPressPlus ? 0.5 : 1}>
			<Wrapper
				shadow
				style={{
				...
				<View style={{ marginHorizontal: SizeList.base }} _flex>
					...
					<Text
						style={[
							min_stock
								? parseInt(stock) <= parseInt(min_stock)
									? { color: ColorsList.danger }
									: stock - quantity <= min_stock
									? { color: ColorsList.danger }
									: null
								: null,
						]}>
						{stock ? `Stok : ${stock}` : "Fitur stok tidak aktif"}
					</Text>
					<Text>{price}</Text>
				</View>
				{right ? (
					...
					<View>
						<TouchableOpacity
							...
							style={styles.cardPlusMinusIcon}>
							<Image
			...
			<TouchableOpacity
				...
				{props.right ? (
					...
					<View
						...
						<Text font="SemiBold" style={{ marginHorizontal: 8 }}>
