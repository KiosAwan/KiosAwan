	...
	return (
		<View style={{ flex: 1, backgroundColor: ColorsList.authBackground }}>
			<AwanPopup.Title
				...
				message={`Dana sebesar ${convertRupiah(
					amount_cancel,
				...
				<Button onPress={_handleCancelTransaction} style={{ width: "25%" }}>
					Ya
				</Button>
			</AwanPopup.Title>
			<AwanPopup.Alert
					...
					<View>
						<View
							...
							<WrapperItem
								...
								left={[
									<Text color="primaryColor" size={24}>
										{convertRupiah(
											dataTransaksi.transaction.status == 1
												? parseInt(amount_cancel)
												: amount_cancel,
										)}
									</Text>,
								]}
								right={
									<Text onPress={() => setDetailItem(!detailItem)} size={16}>
										DETAIL
									</Text>
								}
							/>
							{detailItem
								? dataTransaksi.details_item.rMap((data, i) => {
										return (
											<WrapperItem
												key={i}
												style={{
												...
												right={[
													<Text></Text>,
													<Text style={{ color: ColorsList.greyFont }}>
														{convertRupiah(data.price * data.qty)}
													</Text>,
												]}
											/>
										)
							...
							{detailItem ? (
								<View>
									<WrapperItem
										...
										left={[<Text>Subtotal</Text>]}
										right={
											<Text>
												{convertRupiah(dataTransaksi.transaction.sub_total)}
											</Text>
										}
									/>
									<WrapperItem
										...
										left={[<Text>Pembatalan transaksi</Text>]}
										right={
											<Text color="danger">
												- {convertRupiah(amount_cancel)}
											</Text>
										}
									/>
								</View>
						...
						<FlatList
							...
							renderItem={({ item }) => (
								<ReturnTransactionCard
									...
									minusDisabled={
										dataTransaksi.transaction.discount ? true : false
									}
