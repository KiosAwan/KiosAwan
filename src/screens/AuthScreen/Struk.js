...
const Struk = ({ navigation }) => {
	...
	return (
		<View style={{ flex: 1 }}>
			{response == undefined ? (
				<Spinner color={ColorsList.primary} />
			) : (
				<View style={{ flex: 1 }}>
					<LinearBackground
						content={
							<View>
								<Image
									...
									source={require("../../assets/images/successtransaction.png")}
								/>
								<Text
									style={{
					...
					<View style={{ flex: 1, margin: 15 }}>
						<View style={{ margin: 20 }}>
							<RowOpposite
								title="Pembayaran"
								content={
									response.id_payment_type == 1
										? "Tunai"
										: response.id_payment_type == 2
										? `NonTunai(${response.print.method})`
										: "Piutang"
								}
							/>
							<RowOpposite
								title="Total Tagihan"
								content={convertRupiah(response.total_payment)}
							/>
							<RowOpposite
								title="Tunai Diterima"
								content={convertRupiah(response.amount_payment)}
							/>
							<RowOpposite
								title="Kembalian"
								content={convertRupiah(response.change_payment)}
							/>
						</View>
								...
								<TouchableOpacity
									onPress={() =>
										navigation.navigate("/drawer/transaction/cetakstruk", {
											data: {
												transaction: response.print,
												details_item: response.items,
											},
										})
									}>
									<View style={[styles.wrapIconText]}>
