...
const PhoneRegistration = ({ navigation }) => {
	...
	const _handleLanjutButton = async () => {
		...
		openOtp({
			...
			info:
				"Anda harus memasukkan kode OTP yang telah dikirim ke nomor HP anda",
			resend: _sendOTP,
			onResolve: async otp => {
				setLoading(true)
				const data = {
	...
	return (
		<Container style={{ padding: SizeList.bodyPadding, flex: 1 }}>
			<AwanPopup.Alert
			...
			<Modal
				visible={popup}
				style={{ borderRadius: 5, width: "80%", ...$Padding(20, 20) }}>
				<Text
					style={{
						marginBottom: 10,
						padding: 10,
					...
					size={20}
					align="center">
					0 {FormRegister.phone_number.split("").join(" ")}
				</Text>
				<Text style={{ marginBottom: 10 }} color="pink">
					Nomor ini tidak ditemukan.
				</Text>
				<Text style={{ marginBottom: 10 }}>
					Apakah Anda yakin akan mendaftar aplikasi menggunakan nomor ini?
				</Text>
				<Wrapper style={{ marginTop: 15 }} flexContent>
					<Button onPress={() => setPopup(false)} color="link">
						BATAL
					</Button>
					<Button radius={50} onPress={() => _handleLanjutButton()}>
						LANJUT
					</Button>
				</Wrapper>
			</Modal>
			<View style={{ justifyContent: "center", marginBottom: 10, flex: 1 }}>
				...
				<Text align="center">
					...
					<Text
						color="pink"
						onPress={() =>
							navigation.navigate("/unauth/registration/term-condition")
						}>
						{Strings.REGISTERTERM2}
					</Text>
				</Text>
			</View>
			<Button
