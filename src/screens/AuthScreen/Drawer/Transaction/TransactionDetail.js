...
const TransactionDetail = ({ navigation }) => {
	...
	return (
		<Container
			header={{
				title: "Struk Belanja",
				renderLeftAccessory: fromCashier
					? () => <View style={{ width: 60 }} />
					: undefined,
				onPressBack: () =>
					fromCashier
			...
			<Body>
				...
				{!dataLoading && (
					<ViewShot
						ref={ref => (viewShotRef = ref)}
						options={Config.viewShotOpt()}
						style={{ backgroundColor: ColorsList.authBackground }}>
						<TearLines
						...
						<View
							...
							<Text font="SemiBold" align="center">
								{data ? data.transaction.name_store : null}
							</Text>
							<View
								style={{
								...
								<InfoWrapper>
									...
									<Text>
										{data.transaction.id_payment_type == 1
											? "Tunai"
											: data.transaction.id_payment_type == 2
											? `Non Tunai(${data.transaction.method})`
											: "Piutang"}
									</Text>
								</InfoWrapper>
								<InfoWrapper>
							...
							{data.debt ? (
								<View>
									...
									<View
										...
										<RowOpposite
											title="Jumlah hutang"
											content={convertRupiah(data.debt.total)}
										/>
										<RowOpposite
											title="Jumlah yang sudah dibayar"
											content={convertRupiah(data.transaction.amount_payment)}
										/>
										<RowOpposite
											title="Sisa hutang"
											content={convertRupiah(data.debt.remaining_debt)}
										/>
										<RowOpposite
											style={
											...
											title="Jatuh Tempo"
											content={formatToDays(data.debt.due_debt_date)}
										/>
									</View>
								</View>
							) : null}
							<View
								...
								<View
									name="Daftar Produk"
									style={{
										display: data.details_item.length == 0 ? "none" : "flex",
									}}>
									<View
										style={{
											paddingVertical: 5,
											...$Border(ColorsList.greyAuthHard, 0, 0, 1),
										}}>
										...
										return (
											<Wrapper
												...
												<View _width="76%">
													...
													<Text>
														{convertRupiah(data.price)} x {data.qty}
													</Text>
												</View>
								...
								<View
									name="Daftar Produk Digital"
									style={{
										display: data.product_digital.length == 0 ? "none" : "flex",
									}}>
									<View
										style={{
											paddingVertical: 5,
											...
											<View key={i.toString()}>
												...
												<Wrapper
													...
													<View>
														...
														{item.payment && item.payment.product_name && (
															<Text>{item.payment.product_name}</Text>
														)}
														{item.payment && item.payment.nama && (
															<Text>{item.payment.nama}</Text>
														)}
												...
												{item.transaction.transaction_name == "pln_prepaid" &&
													item.transaction.status == "SUCCESS" && [
														<View>
								...
								<Wrapper
									...
									<Text>{convertRupiah(data.transaction.amount_payment)}</Text>
								</Wrapper>
								<Wrapper
									style={[
										{
								...
								{data.transaction.status != 1 && (
									<Wrapper
										...
										<Text color="danger">
											- {convertRupiah(data.transaction.total_return)}
										</Text>
									</Wrapper>
								)}
								{data.transaction.note !== "" && (
									<View
										style={[
											$Padding(10, 0),
											$Border(ColorsList.authBackground, 0, 0, 1),
										]}>
							...
							<View
								...
								<Image
									...
									source={require("src/assets/images/logostruk.png")}
								/>
							</View>
						</View>
						<TearLines
				...
				data.transaction.status_payment == 2 ? (
					<Wrapper justify="space-between">
						{_canBatal() && (
							<Button
								...
								_width="49%"
								onPress={() =>
									navigation.push("/drawer/transaction/detail/batalkan", {
										paramData: data,
									})
								}>
						...
						<Button
							_width="49%"
							onPress={async () => {
								navigation.push("/drawer/transaction/detail/lunasi", {
									paramData: data,
								})
							}}>
							LUNASI
					...
					<Wrapper flexContent>
						{_canBatal() && (
							<Button
								onPress={() =>
									navigation.navigate("/drawer/transaction/detail/batalkan", {
										paramData: data,
									})
								}
								color="link">
						...
						{fromCashier && (
							<Button onPress={_navigateFromCheckout}>SELESAI</Button>
						)}
					</Wrapper>
				)}
			</Footer>
