...
const UbahEmailInfoScreen = ({ navigation }) => {
	...
	const _nextBtn = async () => {
		...
		openOtp({
			...
			info:
				"Untuk mengubah email, anda harus memasukkan kode OTP yang telah dikirim ke nomor HP anda",
			resend: _sendCode,
			onResolve: async otp => {
	...
	return (
		<View style={{ flex: 1, backgroundColor: ColorsList.authBackground }}>
				...
				{User.data.status == 0 ? (
					...
					<View
						...
						<Text style={{ padding: 10 }} color="settingFont">
							Email anda telah terverifikasi, tekan tombol dibawah untuk
							mengganti
						</Text>
					</View>
				)}
			...
			<Bottom>
				{User.data.status == 0 ? (
					[
						<Button onPress={_handleSendEmail} color="link" width="47.5%">
							KIRIM ULANG
						</Button>,
						<Button
							onPress={() =>
