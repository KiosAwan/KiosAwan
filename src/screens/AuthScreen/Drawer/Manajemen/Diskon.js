...
const ManajemenDiskon = ({ navigation }) => {
	...
	return (
		<View style={{ flex: 1, backgroundColor: ColorsList.authBackground }}>
			<GlobalHeader onPressBack={() => navigation.goBack()} title="Diskon" />
			<View style={{ padding: 20, flex: 1 }}>
				...
				/>
				<FlatList
					data={Discount.data.filter(item =>
						item.name_discount.toLowerCase().includes(search.toLowerCase()),
					)}
					renderItem={({ item, index }) => (
						<View>
							<ManagementPelangganCard
								onPressEdit={() =>
									navigation.navigate("/drawer/manajemen/diskon/edit", { item })
								}
								name={`Potongan Harga ${
									item.discount_type == 1
										? item.value * 100 + " %"
										: convertRupiah(item.value)
								}`}
