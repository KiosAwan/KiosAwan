...
import {
	addDiscountProductPersen,
	ChangeCartQuantity,
	RemoveCartProduct,
	AddDiscountRupiah,
	addDiscountProductRupiah,
	AddDiscountPersen,
...
import {
	...
	RoundedToggleButton,
	PilihPelanggans,
} from "../../components/Picker/SelectBoxModal"
...
const Cart = ({ navigation }) => {
	...
	const _handleChangeDiscountItem = text => {
		if (toggle == 0) {
			if (
				text > 0 &&
				text < parseInt(pesanan.price_out_product) * parseInt(pesanan.quantity)
			) {
				dispatch(
					addDiscountProductRupiah({ item: pesanan, besar_diskon: text }),
				)
			} else {
			...
			if (text > 0 && text < 100) {
				dispatch(
					addDiscountProductPersen({ item: pesanan, besar_diskon: text }),
				)
			} else {
		...
		<Container
			header={{
				...
				renderRightAccessory: () => (
					<TouchableOpacity onPress={() => _emptyCart()}>
			...
			<AwanPopup.NoTitle visible={hapusPesananOpen} message={confirm.title}>
				<Button width="35%" onPress={confirm.action} color="link">
					YA
				</Button>
				<Button width="35%" onPress={() => setHapusPesananOpen(false)}>
					TIDAK
				</Button>
			</AwanPopup.NoTitle>
			<Modal style={{ padding: 10 }} visible={editPesananOpen}>
				<Wrapper
					style={{
						...
						borderBottomWidth: 3,
						borderBottomColor: ColorsList.authBackground,
					}}
					justify="space-between">
					<View width="70%">
						<Text style={{ color: ColorsList.primaryColor, fontSize: 15 }}>
							{pesanan.name_product}
						</Text>
						<Text style={{ color: ColorsList.greyFont }}>
							{convertRupiah(pesanan.price_out_product)} x {pesanan.quantity}
						</Text>
					</View>
					<Text
						_style={{ width: "30%", alignItems: "flex-end" }}
						style={{ textAlignVertical: "center", color: ColorsList.greyFont }}>
						{convertRupiah(
							Number(pesanan.price_out_product) * pesanan.quantity,
						)}
					</Text>
				</Wrapper>
				<Wrapper justify="space-between">
					<MDInput
						...
						value={
							editPesananOpen
								? toggle == 0
									? pesanan.discount_total.toString()
									: pesanan.discount_persen.toString()
								: null
						}
					/>
					<ToggleButton
				...
				<Wrapper justify="space-around" style={{ marginTop: 20 }}>
					<TouchableOpacity
						onPress={() => _quantityControl("min")}
						_style={{ width: "40%", alignItems: "flex-end" }}
						style={{ fontSize: 50, color: ColorsList.primaryColor }}>
						<Image
							size={45}
							source={require("src/assets/icons/minusedit.png")}
						/>
					</TouchableOpacity>
					<TextInput
						_width="10%"
					...
					/>
					<TouchableOpacity
						onPress={() => _quantityControl("add")}
						_style={{ width: "40%", alignItems: "flex-start" }}
						style={{ fontSize: 50, color: ColorsList.primaryColor }}>
						<Image
							size={45}
							source={require("src/assets/icons/plusedit.png")}
						/>
					</TouchableOpacity>
				</Wrapper>
				<Wrapper justify="flex-end" style={{ marginTop: 20 }}>
					<Button color="link" onPress={() => setEditPesananOpen(false)}>
						BATAL
					</Button>
					<Button
						onPress={() => {
			...
			<Body showsVerticalScrollIndicator={false}>
				...
				<View
					...
					{Product.belanja.rMap((data, i) => {
						return (
							<View>
								<Wrapper
									key={i}
									style={{ paddingVertical: SizeList.base }}
									justify="space-between">
									<View _width="68%">
										<Text
											font="SemiBold"
											color="primary"
											size={15}
											style={{ marginBottom: SizeList.secondary }}>
											{data.name_product}
										</Text>
										<Text style={{ color: ColorsList.greyFont }}>
											{convertRupiah(data.price_out_product)} x {data.quantity}
										</Text>
										{data.discount_total == 0 ? null : (
											<Text style={{ color: ColorsList.greyFont }}>
												Diskon{" "}
												{data.discount_rupiah
													? convertRupiah(data.discount_total)
													: data.discount_persen + "%"}
											</Text>
										)}
									</View>
									<View _width="30%">
										<View
											style={{
												marginBottom: SizeList.secondary,
												width: "70%",
												alignSelf: "flex-end",
											}}>
											<Wrapper align="flex-end" justify="space-between">
												<TouchableOpacity
													activeOpacity={0.5}
													onPress={() => _prompDeletePesanan(i, data)}
															...
															<View
																...
																<View>
																	<Wrapper spaceBetween>
																		...
																		<Text _flexEnd>
																			{convertRupiah(
																				Number(pesanan.price_out_product) *
																					pesanan.quantity,
																			)}
																		</Text>
																	</Wrapper>
																	<Input
																		...
																		onChangeText={_handleChangeDiscountItem}
																		style={{
																			paddingVertical: SizeList.base * 2,
																		}}
																		value={
																			toggle == 0
																				? pesanan.discount_total
																					? pesanan.discount_total.toString()
																					: ""
																				: pesanan.discount_persen
																				? pesanan.discount_persen.toString()
																				: ""
																		}
																		renderRightAccessory={() => (
																	...
																	<Wrapper>
																		...
																		<Input
																			noLabel
																			_flex
																			onChangeText={_quantityControl}
																			value={
																				pesanan.quantity
																					? pesanan.quantity.toString()
																					: ""
																			}
																			keyboardType="number-pad"
																...
																<Wrapper flexContent>
																	<Button color="link" onPress={close}>
																		BATAL
																	</Button>
																	<Button
																		onPress={() => {
										...
										<Text align="right">
											{convertRupiah(data.price_out_product * data.quantity)}
										</Text>
										{data.discount_total == 0 ? null : (
											<Text style={{ color: ColorsList.greyFont }}>
												{convertRupiah(data.discount_total)}
					...
					{Product.ppob_cart.length > 0 ? <Text>Payment Point</Text> : null}
							...
							<View>
								<Wrapper
									...
									<View _width="60%">
										<Text
											font="SemiBold"
											style={{ color: ColorsList.primaryColor, fontSize: 15 }}>
					...
					{Product.total_diskon == 0 ? null : (
						<View>
							<Wrapper justify="space-between">
								...
								<Text style={{ paddingVertical: 10 }} color="danger">
									- {convertRupiah(Product.total_diskon)}
								</Text>
							</Wrapper>
							<Divider />
						</View>
					...
					<Wrapper justify="space-between">
						<Text style={{ paddingVertical: 10 }} font="SemiBold">
							Total
						</Text>
						<Text style={{ paddingVertical: 10 }} font="SemiBold">
							{convertRupiah(Product.total - Product.total_diskon)}
						</Text>
					</Wrapper>
				</View>
				...
				<View style={{ marginVertical: SizeList.base }}>
					...
					<PilihPelanggan
						value={
							Product.customer
								? Product.customer.name_customer
					...
					{Product.discount_on ? (
						<Wrapper justify="space-between" style={{ paddingVertical: 10 }}>
							<Input
								...
								value={
									discount_type == 0
										? Product.discount_total_rupiah
										: Product.discount_total_persen
								}
				...
				<Input
					value={Product.note}
					onChangeText={text => {
						dispatch(addTransactionNote(text))
					}}
					label="Catatan Pembelian"
					placeholder="Masukkan catatan pembelian disini"
				/>
			</Body>
			<Footer>
				<Button
