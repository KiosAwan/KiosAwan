...
const AddCartWithBarcode = ({ navigation }) => {
	...
	const _onBarCodeRead = async scanResult => {
		...
		let barcodeProduct = Product.data.find(
			item => barcode == item.barcode_product,
		)
		if (barcodeProduct) {
			if (barcodeProduct.manage_stock == 1) {
				if (
					parseInt(barcodeProduct.stock) -
						parseInt(barcodeProduct.quantity || 0) ==
					0
				) {
					Alert.alert(
						"",
	...
	const _saveProduct = () => {
		dispatch(
			AddCartByBarcode({
				barcode: productData.barcode_product,
	...
	const _handlePlusQuantity = () => {
		if (productData.manage_stock == 1) {
			if (
				productQuantity <
				parseInt(productData.stock) - parseInt(productData.quantity || 0)
			) {
	...
	return (
		<View style={{ flex: 1 }}>
			...
			{modalVisible ? (
				<Modal animation="fade" style={{ padding: 10 }} visible={modalVisible}>
					<Wrapper
						...
						<View width="70%">
							...
							<Text style={{ color: ColorsList.greyFont }}>
								{convertRupiah(productData.price_out_product)} x{" "}
								{productQuantity}
							</Text>
						</View>
						<Text
							...
							{convertRupiah(
								Number(productData.price_out_product) * productQuantity,
							)}
						</Text>
					</Wrapper>
					<Wrapper justify="space-around" style={{ marginTop: 20 }}>
						<TouchableOpacity
							onPress={_handleMinusQuantity}
					...
					<Wrapper justify="flex-end" style={{ marginTop: 20 }}>
						<Button color="link" onPress={_handleKembali}>
							KEMBALI
						</Button>
						<Button onPress={_handleLanjutScan} style={styles.buttonSimpan}>
							<Text size={12} style={{ color: "white" }}>
								LANJUT SCAN
							</Text>
						</Button>
					</Wrapper>
			...
			<RNCamera
				style={styles.camera}
				onBarCodeRead={
					scanWork ? _onBarCodeRead : () => console.debug("SCAN DISABLE")
				}
				defaultTouchToFocus
				onFocusChanged={() => {}}
				ratio="1:1"
