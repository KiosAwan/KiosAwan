...
const Emoney = ({ navigation }) => {
    ...
    return <Container>
        <GlobalHeader onPressBack={() => navigation.goBack()} title="E-Money" />
        <View style={styles.topComp}>
            <View onLayout={_layout}>
                <TouchableOpacity onPress={() => setDropdownVisible(true)}>
                    <View style={styles.selectContainer}>
                        <Wrapper justify="space-between" style={styles.selectWrapper}>
                            <Text size={16}>{selected ? selected.name : "Pilih E-Money"}</Text>
            ...
            <AwanPopup.Menu noTitle transparent absolute visible={dropdownVisible}
                ...
                style={[styles.dropdownStyle, { width: "100%", top: Object.keys(nativeEvent).length > 0 ? nativeEvent.layout.y + nativeEvent.layout.height * 2 + 20 : 200 }]}
            ...
            <FlatList style={styles.listPulsa} numColumns={2} keyExtractor={(a, i) => i.toString()}
                ...
                renderItem={({ item, index }) =>
                    <TouchableOpacity onPress={() => _selectPulsa({ item, index })} style={[styles.pulsaWrapper, index === selected && styles.pulsaWrapperActive]}>
                        ...
                        <Text color="primary" size={20} style={styles.pulsaComp}>{convertRupiah(20000)}</Text>
                        <Divider />
                        <Text style={styles.pulsaComp}>Harga: {convertRupiah(21000)}</Text>
                    </TouchableOpacity>
                }
            />
        </ContainerBody>
