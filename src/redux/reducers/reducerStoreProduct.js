...
const reducerStoreProduct = (state = initialState, actions) => {
	switch (actions.type) {
		...
		case "ADD_BY_BAROCDE":
			...
			let barcodeProduct = state.data.find(
				item => barcode == item.barcode_product,
			)
			let barcodeExistedItem = state.belanja.find(
				item => barcode == item.barcode_product,
			)
			if (barcodeExistedItem) {
				;(barcodeProduct.quantity += barcodeQty),
					(barcodeProduct.total +=
						barcodeProduct.price_out_product * barcodeQty)
				state.jumlahitem += barcodeQty
				if (!barcodeProduct.discount_rupiah) {
					barcodeProduct.discount_total +=
						(parseInt(barcodeProduct.discount_persen) / 100) *
						parseInt(barcodeProduct.price_out_product)
					return {
						...state,
						total:
							state.total +
							parseInt(barcodeProduct.price_out_product) * parseInt(barcodeQty),
						total_diskon:
							state.total_diskon +
							(parseInt(barcodeProduct.discount_persen) / 100) *
								parseInt(barcodeProduct.price_out_product) *
		...
		case "ADD_TO_CART_MANUAL":
			...
			newBelanja.total =
				newBelanja.price_out_product * parseInt(newBelanja.quantity)
		...
		case "CHANGE_QUANTITY_MANUAL":
			...
			const itemDimaksud = state.belanja.find(
				item => product_data.id_product === item.id_product,
			)
			...
			if (state.discount_total_persen > 0) {
				manual_trx_discount =
					(parseInt(state.discount_total_persen) / 100) *
					parseInt(itemDimaksud.price_out_product) *
					parseInt(product_data.quantity - temp_item_quantity)
			...
			if (!itemDimaksud.discount_rupiah) {
				itemDimaksud.total =
					itemDimaksud.price_out_product * parseInt(itemDimaksud.quantity)
				let newTotalBaru =
					state.total +
					itemDimaksud.total -
					temp_item_quantity * itemDimaksud.price_out_product
				itemDimaksud.discount_total =
					(parseInt(itemDimaksud.discount_persen) / 100) *
					parseInt(itemDimaksud.price_out_product) *
					parseInt(itemDimaksud.quantity)
				return {
					...state,
					total: newTotalBaru,
					total_diskon:
						...
						(parseInt(itemDimaksud.discount_persen) / 100) *
							parseInt(itemDimaksud.quantity) *
							parseInt(itemDimaksud.price_out_product) -
						parseInt(temp_discount_total) +
						parseInt(manual_trx_discount),
					belanja: [...state.belanja],
				}
			} else {
		...
		case "REMOVE_SELECTED_PRODUCT":
			...
			const removedItem = state.belanja.find(
				item => removedProduct.id_product === item.id_product,
			)
			const temp_rmvitem_quantity = removedItem.quantity
			const temp_rmvitem_price = removedItem.price_out_product
			let newTotalRemove =
				state.total -
				parseInt(temp_rmvitem_quantity) * parseInt(temp_rmvitem_price)
			let notRemoveBelanja = state.belanja.filter(
				item => item.id_product != removedItem.id_product,
			)
		...
		case "REMOVE_PRODUCT_CART":
			let total_online = 0
			state.ppob_cart.forEach(
				item => (total_online = total_online + parseInt(item.price)),
			)
			return {
				...state,
				total: total_online,
		...
		case "ADD_DISCOUNT_PERSEN":
			...
			return {
				...state,
				discount_transaction: parseInt(
					persenDisc == "" ? 0 : (persenDisc / 100) * state.total,
				),
				total_diskon:
					state.total_diskon +
					parseInt(persenDisc == "" ? 0 : (persenDisc / 100) * state.total) -
					parseInt(temp_tex_persen_disc),
		...
		case "ADD_DISCOUNT_RUPIAH":
			...
			return {
				...
				total_diskon:
					state.total_diskon +
					parseInt(rupiahDisc == "" ? 0 : rupiahDisc) -
					parseInt(temp_trx_disc),
			}
		...
		case "QUANTITY_INCREMENT":
			...
			const itemMauDitambah = state.belanja.find(
				item => itemTambah.id_product === item.id_product,
			)
			if (itemMauDitambah) {
				...
				if (state.discount_total_persen > 0) {
					plus_trx_discount =
						(parseInt(state.discount_total_persen) / 100) *
						parseInt(itemTambah.price_out_product)
					state.discount_transaction += plus_trx_discount
				}
				if (!itemMauDitambah.discount_rupiah) {
					itemMauDitambah.discount_total +=
						(parseInt(itemMauDitambah.discount_persen) / 100) *
						parseInt(itemMauDitambah.price_out_product)
					return {
						...state,
		...
		case "ADD_DISCOUNT_PRODUCT_RUPIAH":
			...
			const discountItemRupiah = state.belanja.find(
				item => itemDiskonRupiah.id_product === item.id_product,
			)
			if (discountItemRupiah) {
				...
				return {
					...
					total_diskon:
						state.total_diskon -
						log_diskon +
						parseInt(itemDiskonRupiah.discount_total),
				}
			}
		case "ADD_DISCOUNT_PRODUCT_PERSEN":
			let itemDiskonPersen = actions.payload.item
			let persen_diskon = actions.payload.besar_diskon
			const discountItemPersen = state.belanja.find(
				item => itemDiskonPersen.id_product === item.id_product,
			)
			if (discountItemPersen) {
				let diskon_sebelum =
					itemDiskonPersen.discount_total == ""
						? 0
						: itemDiskonPersen.discount_total
				...
				persen_diskon !== ""
					? (itemDiskonPersen.discount_total =
							(parseInt(persen_diskon) / 100) *
							parseInt(itemDiskonPersen.price_out_product) *
							itemDiskonPersen.quantity)
					: null
				return {
					...state,
					total_diskon:
						parseInt(state.total_diskon) -
						parseInt(diskon_sebelum) +
						parseInt(itemDiskonPersen.discount_total),
				}
			}
		...
		case "QUANTITY_DECREMENT":
			...
			const itemMauDikurang = state.belanja.find(
				item => itemKurang.id_product === item.id_product,
			)
			if (itemMauDikurang) {
				if (itemMauDikurang.quantity != 0) {
					...
					if (state.discount_total_persen > 0) {
						minus_trx_discount =
							(parseInt(state.discount_total_persen) / 100) *
							parseInt(itemMauDikurang.price_out_product)
						state.discount_transaction -= minus_trx_discount
					}
					if (!itemMauDikurang.discount_rupiah) {
						itemMauDikurang.discount_total -=
							(parseInt(itemMauDikurang.discount_persen) / 100) *
							parseInt(itemMauDikurang.price_out_product)
						if (itemMauDikurang.quantity == 0) {
							let a = state.belanja.filter(
								item => item.id_product != itemMauDikurang.id_product,
							)
							return {
								...state,
								total:
									state.total - parseInt(itemMauDikurang.price_out_product),
								total_diskon:
									state.total_diskon -
									(parseInt(itemMauDikurang.discount_persen) / 100) *
										parseInt(itemMauDikurang.price_out_product) -
