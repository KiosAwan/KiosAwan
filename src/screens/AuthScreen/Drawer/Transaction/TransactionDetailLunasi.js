	...
	const _renderKembalian = () => {
		try {
			if (amount_payment.extractNumber() - dataUtang.debt.remaining_debt >= 0)
				return `Kembalian: ${convertRupiah(
					amount_payment.extractNumber() - dataUtang.debt.remaining_debt,
				)}`
		} catch (err) {}
		return null
	}
			...
			<View style={{ padding: 15, flex: 1 }}>
				{route.key == "first" ? (
					<ScrollView showsVerticalScrollIndicator={false}>
						<View
							style={{ padding: 20, backgroundColor: ColorsList.whiteColor }}>
							<Input
								style={{ margin: 0 }}
								currency
								noShadow
							...
							<View style={{ ...RowChild, marginTop: SizeList.base }}>
								<ToggleButtonMoney
									...
									buttons={[
										dataUtang.debt.remaining_debt,
										getNearestFifty(dataUtang.debt.remaining_debt, 1),
									]}
								/>
							</View>
						</View>
				...
				<View
					...
					{nonTunaiList.rMap((item, i) => (
						<TouchableOpacity
							style={[
								styles.card,
								nonTunai == i + 1 ? styles.selectedNonTunai : null,
							]}
		...
		<View style={{ flex: 1, backgroundColor: ColorsList.authBackground }}>
			...
			{loading ? (
				...
				<View style={{ flex: 1 }}>
													...
													<Wrapper direction="column">
														<Text>
															...
															<Text font="SemiBold" color="primary">
																{convertRupiah(dataUtang.debt.remaining_debt)}
															</Text>
														</Text>
													</Wrapper>
												</Wrapper>
					...
					<Bottom>
						<Button onPress={_handlePayCredit} width="100%">
							LUNASI
						</Button>
					</Bottom>
				</View>
