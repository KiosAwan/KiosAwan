		...
		return <View>
			<FlatList
				...
				renderItem={({ item, index }) => {
					return <Button
						onPress={() => { if (item.enabled == 1) { setStep(item) } }} color={item.enabled == 0 ? "grey" : "topupActive"}
						style={{
							flex: 1,
						...
						{
							item != "" && <Image style={{ width: "80%", height: 50, marginHorizontal: 5 }} source={{ uri: `${item.logo}` }} />
						}
					</Button>
				}}
			/>
	...
	const renderStep = () => {
		...
		return <View>
			<View style={{ backgroundColor: ColorsList.white, padding: SizeList.base, borderRadius: SizeList.secondary }}>
				<Wrapper flexStart>
					<Image style={{ width: 50, height: 50 }} source={{ uri: `${logo}` }} />
				...
				<Wrapper spaceBetween>
					<Text _width="80%" size={16} font="SemiBold" style={{ letterSpacing: .7 }}>{noVa}</Text>
