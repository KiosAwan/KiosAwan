...
const MenuSettingProfil = ({ navigation }) => {
	...
	const inputan = [
		...
		},
		{
			...
			onChangeText: address_store => {
				setForm({ address_store })
			},
		},
	]
	...
	const _handleChoosePhoto = () => {
		...
		}).then(({ path: photo_store }) => setForm({ photo_store }))
	}

	const _handleSaveProfile = async () => {
		setApiLoading(true)
		const formData = new FormData()
		let final_address = `${form.address_store}, ${_renderViewAlamat(
			desaSelected,
		)}`
		...
		formData.append(
			"photo_store",
			form.photo_store != ""
				? form.photo_store != temp_profilepic
					? {
	...
	useEffect(() => {
		if (User.store && User.store.address_store) {
			let [
				...
				kabupaten,
				provinsi,
			] = User.store.address_store.split(", ")
			if (provinsi) {
				setDesaSelected({
					id: Math.randomInt(0, 9999),
					desa,
					kecamatan,
					kabupaten,
					provinsi,
			...
			<Body>
				...
				<FlatList
					...
					renderItem={({ item: input }) => (
						<View style={{ marginBottom: 10 }}>
							<Input label={input._label} {...input} />
						</View>
					)}
				/>
				<SelectBoxModal
				...
				{desaSelected.id && (
					<View style={styles.locationDetail}>
						<Text font="SemiBold">{_renderViewAlamat(desaSelected)}</Text>
					</View>
				)}
				<SelectBoxModal
					btnStyle={{ marginTop: 10, marginBottom: SizeList.base }}
					label="Kategori Toko"
					closeOnSelect
