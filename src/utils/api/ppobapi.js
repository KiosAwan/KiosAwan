...
export const inquiryPPOBProduct = async data => {
	...
	try {
		const res = await axios.post(
			`${PPOB_URL}/service/ppob/general/inquiry`,
			data,
			{
				headers: { authorization: userToken },
			},
...
export const paymentPPOBProduct = async data => {
	...
	try {
		const res = await axios.post(
			`${PPOB_URL}/service/ppob/general/payment`,
			data,
			{
				headers: { authorization: userToken },
			},
...
export const getProductPPOBGeneral = async type => {
	...
	try {
		const res = await axios.get(
			`${PPOB_URL}/service/ppob/general/product/${type}`,
			{
				headers: { authorization: userToken },
			},
		)
...
export const getProductPulsa = async data => {
	...
	try {
		const res = await axios.get(
			`${PPOB_URL}/service/ppob/general/product/${data.type}/${data.phone_number}`,
			{
				headers: { authorization: userToken },
			},
		)
