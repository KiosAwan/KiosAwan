            ...
            if (row.address) {
                items.push(
                    <Fragment>
                        <TouchableOpacity key={new Date().getTime() + i} stlye={styles.wtf} onPress={() => {
                            ...
                            <Wrapper style={styles.wrapper}>
                                <View style={{ alignItems: "center", flexDirection: "row", }}>
                                    <Icon size={20} name="bluetooth" color={ColorsList.greyFont} />
                        ...
                        {i != rows.length - 1 && <Divider style={{ marginVertical: SizeList.secondary }} />}
                    </Fragment>
                );
            }
        }
        return <View style={[stylesglobe.shadowView, { padding: SizeList.base }]}>
            {items}
        </View>
    }
    ...
    render() {
        return (
            <View style={{ flex: 1, backgroundColor: ColorsList.authBackground }}>
                <GlobalHeaderWithIcon title="TAMBAH PERANGKAT"
