...
const Button = props => {
	...
	const generateColor = color => {
		...
		for (let key in Colors) {
			if (color === key) _color = Colors[color]
		}
		...
		if (Array.isArray(color)) {
			_color = {
				...
				borderColor:
					color.length === 3 ? ColorsList[color[2]] : ColorsList[color[0]],
			}
		}
		if (borderBottom) {
	...
	return (
		<TouchableOpacity
			...
			style={{
				...
				borderRadius: noRadius ? 0 : radius || 20,
				...(ifWhitespaces() && { display: "none" }),
				...(flex && { flex: 1 }),
				...(["number", "string"].includes(typeof padding)
					? { padding: padding }
					: $Padding(8, 10)),
