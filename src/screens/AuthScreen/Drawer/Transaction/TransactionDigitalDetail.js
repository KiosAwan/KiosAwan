...
const TransactionDigitalDetail = ({ navigation }) => {
				...
				{(payment
					...
					return item != "description" ? (
						...
						<Button
							...
							hideIfEmpty
							disabled>
							<Text color="informationFont" align="center">
	...
	const _renderPendingProductDigital = () => {
		...
		return (
			<View>
				{(transaction
					? Object.keys(transaction).filter(a => !filterPayment.includes(a))
					: []
				).rMap(item => (
					<View>
						<Wrapper spaceBetween style={{ padding: 10 }}>
							...
							<Text align="right" _width="49%">
								{![
									...
									? transaction[item] && transaction[item].trim()
									: parseInt(transaction[item]).convertRupiah()}
							</Text>
						</Wrapper>
					</View>
				))}
					...
					<ViewShot
						...
						{_checkData("status") === "PENDING" ? (
							<Button
								...
								<Text color="whiteColor" style={{ paddingHorizontal: 10 }}>
						...
						<View
							...
							<View style={{ marginVertical: SizeList.base }}>
								...
								<View
									...
									<Wrapper spaceBetween>
										...
										<Text>
											{transaction && transaction.created_at.slice(0, 16)}
										</Text>
									</Wrapper>
									<Wrapper spaceBetween>
										<Text>Operator</Text>
							...
							<View style={{ backgroundColor: ColorsList.whiteColor }}>
								...
								{transaction &&
									transaction.transaction_name == "pln_prepaid" &&
									payment &&
									payment.token && [
										<View>
											<Wrapper
								...
								<Divider />
								{payment
									? _renderProductDigital()
									: _renderPendingProductDigital()}
								<Divider />
								<View
									style={{
										alignItems: "center",
					...
					<Wrapper justify="space-between">
						<Button color="white" _width="49%" onPress={_shareBill}>
							<Image
								...
								source={require("src/assets/icons/share-primary.png")}
							/>
							<Text color="primary" style={styles.btnwithIconText}>
								KIRIM STRUK
							</Text>
						</Button>
						<Button
							...
							<Image
								_style={{ marginRight: 10 }}
								style={{ height: 18, width: 18 }}
								source={require("src/assets/icons/print-primary.png")}
							/>
							<Text color="primary" style={styles.btnwithIconText}>
								CETAK STRUK
							</Text>
