	...
	const inputan = [
		{
			...
			_setEyes: _ => {
				setEyes({ ...eyes, old_password: !eyes.old_password })
			},
			value: formValue.old_password,
			secureTextEntry: !eyes.old_password,
			onChangeText: text => {
				setFormValue({ ...formValue, old_password: text })
			},
		},
		{
			_label: "Password baru",
			_setEyes: _ => {
				setEyes({ ...eyes, new_password: !eyes.new_password })
			},
			value: formValue.new_password,
			secureTextEntry: !eyes.new_password,
			onChangeText: text => {
				setFormValue({ ...formValue, new_password: text })
			},
		},
		{
			_label: "Konfirmasi password baru",
			_setEyes: _ => {
				setEyes({ ...eyes, confirm_new_password: !eyes.confirm_new_password })
			},
			value: formValue.confirm_new_password,
			secureTextEntry: !eyes.confirm_new_password,
			onChangeText: text => {
				setFormValue({ ...formValue, confirm_new_password: text })
			},
		},
	]
	...
	return (
		<View style={{ flex: 1, backgroundColor: ColorsList.authBackground }}>
			...
			<ScrollView
				...
				style={{ padding: SizeList.bodyPadding }}>
							...
							<Input
								...
								renderRightAccessory={() => (
									<Icon
										...
										name={input.secureTextEntry ? "eye" : "eye-off"}
										onPress={input._setEyes}
									/>
								)}
							/>
						</View>
				...
				<View
					...
					<Text style={{ padding: 10 }} color="settingFont">
						Untuk mengganti password, anda harus memasukkan password anda saat
						ini
					</Text>
				</View>
			</ScrollView>
			<Bottom>
				<Button onPress={_handleSavePassword} style={{ width: "100%" }}>
					SIMPAN
				</Button>
			</Bottom>
		</View>
